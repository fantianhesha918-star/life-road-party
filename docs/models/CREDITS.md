...
- **地面ジオメトリの高低差(中央を少し高く・外周をわずかに低く)は2026-08-15に実装・組み込み済み**(`docs/snack-board3d.js`の`terrainHeightAt`/`createIslandGroundGeometry`。地面を同心楕円リングの自作ジオメトリに作り直し、マス・キャラクター・建物等の見た目のY座標もterrainHeightAtに追従させた。ゲームロジック側のx,z座標・当たり判定には一切影響しない)。フェルト芝テクスチャ適用・道路のroad-piece連結配置・ground-clustersの8〜12箇所配置はまだ未実施
