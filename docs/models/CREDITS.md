...
- **2026-08-15、ステージ2のノードグラフ基礎(`docs/snack-stage2-data.js`)を新規着手**(下記「ステージ2ノードグラフ基礎」参照)。この4点の建物・小道具自体はまだ配置未着手(ノードグラフの上に建物を置くのは次回以降のフェーズ)。まだcommit/pushしていない

## ステージ2「シーサイド・アドベンチャー」ノードグラフ基礎(2026-08-15)

- 経緯: `docs/models/`配下に未組み込みのまま残っていた素材を棚卸しする中で、ステージ2向け地形素材(上記4点)の存在から「本当の意味でのステージ2」を作るとどの程度の規模か利用者と確認したところ、Codexが2026-08-11に作成していた詳細設計書(`クロコとcodex受け渡し\素材受け渡し\02_Codex作成素材\アニマルライフ_おやつ集めモード_マップ全体イメージ2種_2026-08-11\ClaudeCode向け_マップ制作引き継ぎ.md`)を発掘。西28+東28+中央橋4〜6ノード(計約60ノード)・建物配置・ワープ・橋の通行止め+罠・ステージ選択画面まで含む、今回の32マス化(第3弾)と同等以上の規模の別プロジェクトだと判明した。利用者と相談の上、**今回は設計書の「制作順」1〜2(ノードIDと接続のデバッグ表示→到達可能性検証)のみに着手範囲を絞り、建物配置・道路曲線・カメラ・ワープ/罠/通行止め・ステージ選択UI・セーブデータ拡張は次回以降に持ち越すことで合意**した。
- 実装: 新規`docs/snack-stage2-data.js`に`buildSeasideStageNodes()`を実装。ステージ1(`snack-data.js`の`buildSnackStageNodes`)と同じノード形状(id/position/zone/nodeType/nextNodeIds/tollCost/snackSpawnCandidate/trap/gaburion)に揃えた。
  - 西ループ`sea-west-00`〜`27`(28ノード、中心x=-22)、東ループ`sea-east-00`〜`27`(28ノード、中心x=+22)、いずれも半径rx=15/rz=12の同心円状の閉じた周回(ステージ1と同じ`buildSnackStageNodes`の楕円配置ロジックを流用)。
  - 橋: `sea-bridge-west`→`sea-hub`→`sea-bridge-east`の3ノード。西ループの東向き最先端(`sea-west-07`)を分岐ノード(`nextNodeIds`2要素: 通常の周回続行+橋への入口)にし、東ループの西向き最先端(`sea-east-21`)へ合流させる、**西→東の一方向のみ**(設計書は双方向を求めているが、このゲームの`nextNodeIds`は片方向専用グラフのため、双方向化・迂回路は次回のワープ/罠/通行止め実装フェーズに合わせて設計する方針。到達可能性の検証には一方向で十分)。
  - 合計59ノード。
- **このファイルはまだゲーム本編に一切組み込まれていない**(`index.html`への`<script>`追加はまだ行っていない、`snack-engine.js`/`snack-board3d.js`/`ui.js`のどこからも参照されない、独立した準備データという位置づけ)。
- 検証: Node vmサンドボックスで`docs/snack-stage2-data.js`を読み込み、①全59ノードのID重複なし、②`nextNodeIds`の参照先が全て実在、③スタート地点(`sea-west-00`)から全59ノードへ到達可能、④西ループ・東ループがそれぞれ橋を使わずに28ノードの閉じた周回になっている、⑤橋3ノードの接続順が正しい、を確認。全項目OK。
- 次回以降のタスク: 建物・道路曲線・道路メッシュの配置、カメラ、ワープ/橋の通行止め+罠の実装(双方向化含む)、ステージ選択UI、セーブデータへのステージID追加。上記4点の地形素材(facility-small-bridge等)の配置もこのタイミングで行う。
...
- **地面ジオメトリの高低差(中央を少し高く・外周をわずかに低く)は2026-08-15に実装を試みたが、地面の見た目を破壊する不具合のため振幅0で無効化中**。`docs/snack-board3d.js`に`terrainHeightAt`/`TERRAIN_CENTER_LIFT`/`TERRAIN_EDGE_DROP`のインフラは残しており、マス・キャラクター・建物等の見た目のY座標もterrainHeightAtに追従する設計は実装済み(ゲームロジック側のx,z座標・当たり判定には一切影響しない設計)。ただし地面メッシュ自体を同心楕円リングの自作ジオメトリ(ドーム状)に作り直したところ、テクスチャを貼ると原因不明のまま白っぽく潰れて表示される不具合が発生し(UV正規化・法線・頂点カラー有無・テクスチャrepeat値などを広く切り分けたが再現条件を特定できず)、セッション内での解決を断念。地面ジオメトリは元のShapeGeometry(完全に平坦)へ戻し、TERRAIN_CENTER_LIFT/TERRAIN_EDGE_DROPを両方0にすることで無効化した。**フェルト芝テクスチャ適用・地区別色ブレンド(vertexColors)は実装・組み込み済み**(`docs/images/ground-grass-felt.jpg`、地区別hex値はterrainZoneColorForLocalXY経由)。道路のroad-piece連結配置・ground-clustersの8〜12箇所配置はまだ未実施
