...
- プレイヤープロフィール(コイン・所持アイテム・装備中の色/帽子/アクセサリー)は**端末のlocalStorageにのみ保存**(Firebaseには送らない)。
- 対局終了時、最終所持金に応じてコインを獲得する(10万円につき1コイン、`shop-data.js`の`GAME_REWARD_MONEY_PER_COIN`で調整可能)。
- 獲得コインでショップ(色・帽子・アクセサリー)を購入し、キャラクター編集画面で装備できる。
...
- 通信モードへのCPU枠混在、演出強化
- アバターアイテムの追加(現状は色6+購入色3、帽子4、アクセサリー4種)
